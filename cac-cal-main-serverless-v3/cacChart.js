...
function addMouseInteraction(path, data, x) {
    path.on("mouseover", (event) => {
        ...
        currentTooltip.html(`${translateText("cac_chart.axis_age")}: 
            ${closestDataPoint.age.toFixed(1)}<br>${translateText("cac_chart.axis_cac")}: 
            ${closestDataPoint.value.toFixed(1)}`)
            .style("left", (event.pageX + 10) + "px")
            .style("top", (event.pageY - 10) + "px");
    })
    .on("mousemove", (event) => {
...
/**
 * This function now ONLY plots the initial (green) point.
 * Prediction points (blue and purple) are plotted directly in updateCACPrediction
 * for better control and clarity, especially with conditional plotting.
 */
...
function addSecondMeasurement(svg, inputData, x, y) {
    ...
    const progressionCategory = determineProgressionCategory(currentPercentile, observedFuturePercentile);

    svg.append("text")
        .attr("x", x(inputData.age_t2) + 10)
        .attr("y", y(inputData.cac_t2) - 10)
        .attr("text-anchor", "start")
        .style("font-size", "12px")
        .style("fill", progressionCategory === 'Rapid' ? "red" : progressionCategory === 'Slow' ? "green" : "blue")
        .style("font-weight", "bold")
        .text(`${progressionCategory} ${translateText("cac_chart.legend_progression_cac")}`);
}
