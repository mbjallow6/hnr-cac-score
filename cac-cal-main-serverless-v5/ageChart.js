...
function updateAgePrediction(inputData) {
    ...
    // Calculate the y-offset for the confidence interval legend item
    const legendItemYOffset = (CAC_LEVELS.length + 1) * 20;

    // Add legend for confidence interval
    legend.append("rect")
        .attr("x", 0)
        .attr("y", legendItemYOffset)
        .attr("width", 10)
        .attr("height", 10)
        .attr("fill", "red")
        .attr("opacity", 0.2);
    ...
    legend.append("text")
        .attr("x", 20)
        .attr("y", legendItemYOffset + 10)
        .style("font-size", "12px")
        .attr("alignment-baseline", "middle")
        .text(translateText("age_chart.legend_confidence_interval"));
